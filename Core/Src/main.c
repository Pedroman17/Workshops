...
int main(void)
{
  ...
  {
    /* USER CODE END WHILE */
	  uint8_t rcvBuf[1]; // buffer for 1 byte
	  HAL_StatusTypeDef result;

	  result = HAL_UART_Receive(&huart3, rcvBuf, 1, 10); // read 1 byte, time out 10 ms

	  if (result == HAL_OK) // if data received
	  {
		  switch (rcvBuf[0]) // process the command
		  {
		  	  case '1': // turn on green led
