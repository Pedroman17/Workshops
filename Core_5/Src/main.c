...
/* Melody definition */
const soundToneType melody[] =
{
    C4, C4, D5, C4, F5, E5,
    C6, C6, G6, F6, E6, D6, C6
};
...
/* Play melody exactly 3 times with sequential LEDs */
void Play_Melody(void)
{

    for(uint8_t cycle = 0; cycle < 3; cycle++)        // 3 repeats
    {
        for(uint8_t i = 0; i < sizeof(melody)/sizeof(melody[0]); i++)
        {
            // Sequential light music effect
            ...
            CS43L22_Beep(melody[i], durations_ms[i]);
            HAL_Delay(20);
        }
    }

    // Turn off all LEDs and beep after finishing
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15, GPIO_PIN_RESET);
}
...
int main(void)
{
  ...
  // Start I2S (required for CS43L22)
  HAL_I2S_Transmit_DMA(&hi2s3, (uint16_t *)dataI2S, 100);

  /* USER CODE END 2 */
  // Play melody ONLY 3 times and stop forever
  Play_Melody();
  /* Infinite loop */
